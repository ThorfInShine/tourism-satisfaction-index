...
class SmartEnsembleH5Fixed:
    ...
    def set_dynamic_weights(self):
        ...
        if len(available_models) == 3:
            ...
            self.ensemble_weights = [0.4, 0.2, 0.4]  # TF-IDF, RF, LSTM
    ...
    try:
        ...
        if 'rating' in df_processed.columns:
            ...
            charts['rating_dist'] = json.dumps({
                ...
                'layout': {
                    ...
                    'showlegend': False,
                    'margin': {'l': 70, 'r': 40, 't': 40, 'b': 90}  # Increased margins
                }
            })
        ...
        if 'sentiment' in df_processed.columns:
            ...
            charts['sentiment_dist'] = json.dumps({
                ...
                'layout': {
                    'title': '',
                    'margin': {'l': 50, 'r': 50, 't': 40, 'b': 70}  # Better margins
                }
            })
            ...
            try:
                ...
                charts['top_rating'] = json.dumps({
                    ...
                    'layout': {
                        ...
                        'showlegend': False,
                        'height': 400,
                        'margin': {'l': 220, 'r': 50, 't': 30, 'b': 90}  # Increased margins
                    }
                })
            except Exception as e:
...
def dashboard():
    try:
        ...
        if df_processed is None:
            if not initialize_app():
                flash('Silakan upload data terlebih dahulu', 'error')
...
@app.route('/admin')
def admin_page():
    """Admin page for managing all data"""
    try:
        kunjungan_data = get_kunjungan_data()
...
def add_wisata():
    """Add new wisata with visit count"""
    try:
        data = request.json
...
def update_wisata():
    """Update existing wisata visit count"""
    try:
        data = request.json
        nama_wisata = data.get('nama_wisata', '').strip()
        jumlah_kunjungan = int(data.get('jumlah_kunjungan', 0))
...
def api_filter_data():
    ...
    try:
        ...
        if 'rating' in filtered_df.columns:
            ...
            filtered_charts['rating_dist'] = json.dumps({
                'data': [{
                    'x': [str(x) for x in rating_counts.index.tolist()],
                    'y': rating_counts.values.tolist(),
                    'type': 'bar',
        ...
        if 'sentiment' in filtered_df.columns:
            ...
            filtered_charts['sentiment_dist'] = json.dumps({
                'data': [{
                    'labels': sentiment_counts.index.tolist(),
                    'values': sentiment_counts.values.tolist(),
                    'type': 'pie',
...
def export_kunjungan():
    ...
    try:
        ...
        if not kunjungan_data:
            flash('Tidak ada data kunjungan untuk diekspor', 'warning')
            return redirect(url_for('admin_page'))
...
def start_application():
    ...
    try:
        ...
        if FLASK_ENV == 'production':
            ...
            app.logger.info(f"🌍 Open browser to: http://localhost:{port}")
